...
module Sprockets
  ...
  class Bundle
    def self.call(input)
      ...
      process_bundle_reducers(assets, reducers, type).merge(dependencies: dependencies, included: assets.map(&:uri))
    end
    ...
    def self.process_bundle_reducers(assets, reducers, type)
      initial = {}
      ...
      assets.reduce(initial) do |h, asset|
        reducers.each do |k, (_, block)|
          ...
          if value.is_a?(String) && !value.blank? && type == 'text/css'
